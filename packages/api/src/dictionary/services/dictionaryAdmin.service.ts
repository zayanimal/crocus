...
export class DictionaryAdminService {
  ...
  getAdminDictinaries({ dicts }: AdminDictDto): Observable<unknown> {
    return from(
      this.adminDictsRepository
        .createQueryBuilder("adminDictionaries")
        .where("adminDictionaries.name IN (:...name)", {
          name: dicts.split("%"),
        })
        .getMany()
    ).pipe(
  ...
  createAdminDictionary(name: string) {
    return from(this.adminDictsRepository.findOne({ where: { name } })).pipe(
      mergeMap((dictionary) =>
        dictionary?.name
          ? throwError(new BadRequestException("Словарь уже существует"))
          : from(
              this.adminDictsRepository.save(
  ...
  removeAdminDictionary(name: string) {
    return from(this.adminDictsRepository.findOne({ where: { name } })).pipe(
      ...
      switchMap((dictionary) =>
        from(this.adminDictsRepository.remove(dictionary)).pipe(
          map(() => ({ message: `Словарь ${name} удалён` }))
        )
      ),
      catchError(() =>
        throwError(new NotFoundException(`Словарь ${name} не найден`))
      )
    );
  }
}
