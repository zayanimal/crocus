...
export class GoodService implements IGoodService {
  ...
  updatePricelist(buffer: ArrayBuffer, vendor: string) {
    ...
    return from(data as IDlinkRow[]).pipe(
      ...
      mergeMap((row) =>
        this.goodRepository.searchStrict(row.name).pipe(
          map((found) => found || row),
          mergeMap((entity) =>
            entity instanceof Good
              ? of(entity).pipe(
                  tap((value) => foundBuffer.push(value.name)),
                  mergeMap((good) =>
                    foundBuffer.check(good.name)
                      ? of(null)
                      : this.priceService.create(row.price, good)
                  )
                )
              : of(entity).pipe(
                  mergeMap(({ name, price, description }) =>
                    of(this.goodRepository.create({ name })).pipe(
                      mergeMap((createdGood) =>
                        from(this.goodRepository.save(createdGood)).pipe(
                          catchError(() =>
                            from(
                              this.goodRepository.save(
                                createdGood.setDublicate()
                              )
                            )
                          )
                        )
                      ...
                      mergeMap((newGood) =>
                        forkJoin([
                          ...
                          mergeMap(([descr]) =>
                            this.goodRepository.setDescription(newGood, descr)
                          )
                        )
                      )
                    )
