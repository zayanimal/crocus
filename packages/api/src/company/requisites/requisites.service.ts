...
export class RequisitesService {
  ...
  update(requisitesDto: RequisitesDto[], companyId: string) {
    return of(requisitesDto).pipe(
      mergeMap((requisites) =>
        requisites.length
          ? from(this.requisitesRepository.find({ companyId })).pipe(
              mergeMap((requisites) =>
                from(requisites).pipe(
                  filter((item) =>
                    requisitesDto.some(({ id }) => item?.id !== id)
                  ),
                  mergeMap((filtredItem) =>
                    from(this.requisitesRepository.remove(filtredItem))
                  ),
      ...
      mergeMap((requisites) =>
        from(this.requisitesRepository.findOne(requisites.id)).pipe(
          mergeMap((found) =>
            found
              ? forkJoin([
                  from(
                    this.requisitesRepository.update(
                      { id: requisites.id },
