import { isArray, isObject, merge, set, transform } from 'lodash'
import { from, of, throwError } from 'rxjs'
import { map, mergeMap } from 'rxjs/operators'
import { validate, ValidationError } from 'class-validator'
import { classToPlain } from 'class-transformer'
import { ValidationService } from '@shared/types'
import { ValidationEntity } from '@shared/entities'

export class ValidationServiceImp implements ValidationService {
    private entity = {}
    ...
    private parse(messages: ValidationError[], buffer = {}): object {
        return messages.reduce((acc, err) => {
            if (err.constraints) {
                return set(buffer, err.property, this.buildMessage(err.constraints))
            }
