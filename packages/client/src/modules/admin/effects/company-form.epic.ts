...
import { NormalizatorImp } from '@shared/services'
...
const nrmlztr = new NormalizatorImp('requisites')
...
export const updateCompany: Epic = (action$, state$, { company, validation }) =>
    action$.pipe(
        ...
        catchError((err, caught) =>
            merge(
                caught,
                of(companyControlActions.getCompany.success(nrmlztr.normalize(err))),
                of(systemActions.infoNotification('Проверьте ошибки в реквизитах'))
            )
        )
    )
