...
export const sendRequestNewProject: Epic = (action$, state$) =>
  action$.pipe(
    filter(isActionOf(requestActions.sendNewProject)),
    switchMap(() =>
      forkJoin({
        order: state$.pipe(first(), map(requestSelectors.orderForSend)),
        endUser: state$.pipe(first(), map(requestDrawerSelectors.endUserData)),
      }).pipe(first())
    ),
    mergeMap(() => [
...
export const clearRequestDrawer: Epic = (action$) =>
  action$.pipe(
